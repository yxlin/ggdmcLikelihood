#!/usr/bin/env Rscript
# Compare package likelihood vs manual R implementation on SAME data

cat("\n\n-------------- Package vs Manual Implementation ---------------\n")
rm(list = ls())
...
# Manual DINO implementation (same as before)
...
cat("Simulating data using PACKAGE...\n")
set.seed(123)
...
sigma_test <- c(0.10, 0.15, 0.20, 0.25, 0.30)
...
# Prepare manual calculation
P_LJ <- compute_dino_probabilities(profiles, Q, true_guess, true_slip)

# Convert package data to matrix form for manual calculation
...
for (i in seq_along(sigma_test)) {
  ...
  cat(sprintf("  Package: %.4f\n", loglik_pkg))
  cat(sprintf("  Manual:  %.4f\n", loglik_manual))
...
cat(sprintf("\nPackage max at sigma = %.2f\n", results$sigma[max_pkg]))
cat(sprintf("Manual max at sigma = %.2f\n", results$sigma[max_manual]))
cat(sprintf("True sigma = %.2f\n\n", true_sigma))

if (max_pkg != max_manual) {
  cat("*** DIFFERENT MAXIMA! Package gives WRONG answer! ***\n")
  cat("The bug is confirmed to be in the C++ package implementation.\n")
} else {
  cat("Same maximum. Need to test with larger N or more sigma values.\n")
}

# Check if differences are constant (would suggest simple offset)
# or varying (would suggest parameter-dependent bug)
cat("\nDifference pattern:\n")
cat("  Mean difference:", mean(results$difference), "\n")
cat("  SD of differences:", sd(results$difference), "\n")
cat("  Range:", range(results$difference), "\n")

if (sd(results$difference) < 0.01) {
  cat("\n  Differences are nearly constant - suggests simple offset bug.\n")
} else {
  cat("\n  Differences vary with sigma - suggests parameter-dependent bug!\n")
}
